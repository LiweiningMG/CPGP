...
## 参数名
TEMP=$(getopt -o 4hp:b:v:m:o: --long rmNeg,intercept,debug,label:,phef:,bfile:,varf:,pedf:,gmat:,DIR:,rep:,fold:,add_rf:,gen:,year:,iyse:,tbvf:,tbv_col:,phereal:,soft:,binf:,seed:,all_eff:,iter:,burnin:,bin:,thin:,report:,ran_eff:,method:,thread:,miss:,gidf:,add_sol:,invA:,num_int:,code:,alpha:,out:,overWri,valphe,dmu4,help \
  -n 'javawrap' -- "$@")
if [ $? != 0 ]; then
  echo "Terminating..." >&2
  exit 1
...
while true; do
  ...
  -b |--bfile)   bfile="$2";      shift 2 ;; ## plink二进制文件前缀
      --label)   label="$2";      shift 2 ;; ## 品种标志符(基因型文件中的family ID)
  ...
  -m |--gmat)    gmat="$2";       shift 2 ;; ## 用户提供关系矩阵或逆矩阵(id id value)
      ...
      --gen)     gen="$2";        shift 2 ;; ## 有表型个体中倒数gen个世代个体为验证群
      ...
      --method)  method="$2";     shift 2 ;; ## 育种值估计方法，可为PBLUP/GBLUP/ssGBLUP/BayesA [GBLUP]
...
software=${software:=JWAS}                    ## 育种值估计软件
out=${out:=accuracy.txt}                      ## 准确性输出文件名
code=${code:=$(cd "$(dirname "$0")/.." && pwd)}               ## 脚本所在文件夹
...
## 脚本
phe_group=${code}/R/pheno_grouped.R
accur_cal=${code}/R/accur_dmu_bayes_cal.R
...
## 作业线程
[[ ${CPGP_CPUS_ON_NODE} ]] && thread=${CPGP_CPUS_ON_NODE}
    ...
    if [[ ${method} == "BayesAS" ]]; then
      ...
      if [[ ${software} == "C" ]]; then
        job_pool_run mbBayesAS \
          ...
          --seed ${seed} \
          --logf ${bin}_gibs_${CPGP_JOB_ID:-local}.log
      else
        echo "soft ${software} not supported!"
      fi
    elif [[ ${dmu4} ]]; then
      sed 's#$ANALYSE.*#$ANALYSE 11 9 0 0#g' ${workdir}/${DIR}.DIR >${workdir}/val${f}/rep${r}/${DIR}.DIR
      [[ ! ${debug} ]] && job_pool_run run_dmu4 ${DIR} ${workdir}/val${f}/rep${r}
    else
      sed 's#$ANALYSE.*#$ANALYSE 1 1 0 0#g' ${workdir}/${DIR}.DIR >${workdir}/val${f}/rep${r}/${DIR}.DIR
