  ...
  for (f in 1:opt$fold) {
    ...
    if (r == 1 && opt$fold > 1) {
      ...
      if (opt$type == "blend") {
        ...
        nind_ref <- sum(apply(phe[, (ncol(phe) - 1):ncol(phe)], 1, function(x) sum(x == opt$miss) == 1))
      }
...
opt <- list()
opt$pops <- "A B C"
opt$phef <- "/public/home/liujf/liwn/mbGS/QMSim/Four/identical/cor0.1/#breed#/val#val#/rep#rep#/pheno.txt"
opt$rep <- 5
opt$fold <- 2
opt$type <- "union"
opt$pheCol <- 4
opt$overwri <- ""
opt$out <- "/public/home/liujf/liwn/mbGS/QMSim/Four/identical/cor0.1/blend/val#val#/rep#rep#/pheno.txt"
