...
# Output directory name. Joint GBLUP uses separate directories for SGRM and MGRM.
type_dir=${type}
...
## 脚本
MTG2=${code}/shell/MTG2.sh
multiG=${code}/R/multi_pop_Gmat.R
pheMerge=${code}/R/pheno_merge_pop.R
accur_cal=${code}/R/accur_dmu_bayes_cal.R
bins_define=${code}/R/bins_define.R
MCMC_polt=${code}/R/MCMC_chain_plot.R
variance_prior=${code}/R/variance_prior.R
job_pool=${code}/shell/job_pool.sh
ldbolck=${code}/shell/bin_define_ldblock.sh
local_rg=${code}/R/local_block_rg.R

## 并行作业数设置
if [[ ! ${thread} ]]; then
  if [[ ${CPGP_CPUS_ON_NODE} ]]; then thread=${CPGP_CPUS_ON_NODE}; else thread=$((rep * fold)); fi
fi
...
if [[ ${type} == 'multi' ]] && [[ ! -s ${binf} || ${overwrite} ]]; then
  ...
  if [[ ${bin} == "fix" ]]; then
    ...
    $ldbolck \
      ...
      --type ${bin} \
      --minSize ${nsnp_win} \
      --out ${binf}~ >${bin}_${ref}_block_${CPGP_JOB_ID:-local}.log
...
if [[ ${rg_local} && ${bin} == "lava" ]]; then
  ...
  if [[ -s ${summA}.assoc.txt && -s ${summB}.assoc.txt ]]; then
    ...
    $local_rg \
      ...
      --bfile ${!bfile_block} \
      --block ${binf}~ \
      --out ${binf}~ &>${bin}_local_rg_${CPGP_JOB_ID:-local}.log
  else
    ...
    if [[ ${type} == 'multi' ]]; then
      ...
      if [[ ${software} == C ]]; then
        ...
        job_pool_run mbBayesABLD \
          ...
          --seed ${seed} \
          --logf ${bin}_${r2_merge}_${nsnp_win}_gibs_${CPGP_JOB_ID:-local}.log
...
for i in $(seq 0 $((np - 1))); do
  ...
  if [[ ${type} == 'multi' ]]; then
    ...
    [[ ${bin} == 'lava' ]] && outf=${out}_${dirPre}${software}_${bin}_${popN[${i}]}.txt
